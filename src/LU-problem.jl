...
mutable struct LUProblem
    ...
    function LUProblem(model   :: AbstractModel,
        initial :: InitialData,
        times   :: Times;
        solver = LUEulerMaruyama(model)  :: TimeSolver,
        label = nothing
        )
        ...
        #    mesh  = Mesh(param)
        #catch
        #    η,v,x = model.mapfro(U)
        #    mesh = Mesh(x)
        #end

        # A basic check
        try
            STOstep!(solver, model, copy(last(data.U)), 1, 0)
        catch
            @warn "The model and the solver are incompatible. `LUsolve!` will not work."
        end
    ...
    function LUProblem(model   :: AbstractModel,
        ...
        p = LUProblem(model, initial, times; solver = solver, label = label)
...
"""
function LUsolve!(problem :: LUProblem; verbose=true::Bool)
    ...
    if problem.times.tc == problem.times.ts
        ...
        for j in 1:problem.times.Ns-1
            STOstep!(solver, model, U, dt, j .* dt )
            push!(data,copy(U))
            next!(pbar)
        end
...
Information are not printed if keyword argument `verbose = false` (default is `true`).
"""
function LUsolve!(problems; verbose=true::Bool)
    ...
    @threads for i in 1:length(problems)
        ...
        if problems[i].times.tc == problems[i].times.ts
            for j in 1:problems[i].times.Ns-1
                STOstep!(problems[i].solver, problems[i].model, U[i], problems[i].times.dt, j * dt)
...
Base.:(==)(p1::LUProblem, p2::LUProblem) =
    p1.model == p2.model &&
