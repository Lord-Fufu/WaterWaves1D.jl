...
"""
mutable struct LUSerreGreenNaghdi <: AbstractModel
    ...
    function LUSerreGreenNaghdi(param::NamedTuple;
							dealias = 0,
							ktol	= 0,
							iterate	= true,
							gtol	= 1e-14,
		...
		m=LUWhithamGreenNaghdi(param;SGN=true,
							dealias=dealias,
							ktol=ktol,
							iterate=iterate,
							gtol=gtol,
		...
		new(m.label, m.f!, m.mapto, m.mapfro, m.mapfrofull, m.info, m.stoch_incr!, m.n_phi, m.isTrueStrato)
    end
end
