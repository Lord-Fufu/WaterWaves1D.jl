...
"""
mutable struct LUBoussinesq <: AbstractModel
    ...
    function LUBoussinesq(param::NamedTuple;
						a=0,b=0,
						dealias=0,ktol=0,
						label="LU-Boussinesq"
						)

		m=LUWhithamBoussinesq(param;Boussinesq=true,
							dealias=dealias,ktol=ktol,
							label=label)
		
		new(label, m.f!, m.mapto, m.mapfro, m.info, m.stoch_incr!, m.n_phi, m.isTrueStrato, m.smoothingField)
	end
end
